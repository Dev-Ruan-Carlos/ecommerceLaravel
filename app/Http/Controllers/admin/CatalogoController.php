    ...
    public function cadastro(Request $request){
        ...
        $produto->precovenda = (float) preg_replace(['/\./', '/\,/'], ['', '.'], $request->get('precovenda'));
            if($produto->precocusto > $produto->precovenda){
                return redirect()->back()->withInput()->withErrors(['admin.catalogo.cadastro' => 'Preço de venda superior ao preço de custo! TENTE NOVAMENTE']);        
            }
        $produto->precopromocao = (float) preg_replace(['/\./', '/\,/'], ['', '.'], $request->get('precopromocao'));
            if($produto->precopromocao > $produto->precovenda) {
                return redirect()->back()->withInput()->withErrors(['admin.catalogo.cadastro' => 'Preço de promoção superior ao preço de venda! TENTE NOVAMENTE']);        
            }   
        $produto->ativo = 1;
        $produto->save();
